  ...
  async login(username: string, password: string) {
    ...
    if (isFirebaseConfigured && auth) {
      ...
      if (localUsers[username] && localUsers[username] === password) {
        ...
        this.error = 'Invalid username or password in local session.';
        this.loading = false;
...
class CounterStore {
          ...
          if (currentUser) {
            ...
            if (useFirebase) {
              ...
              this.unsubscribeCounters = onSnapshot(qCounters, (snapshot) => {
                ...
                console.error('[CounterStore onSnapshot counters] Error:', error);
                this.loading = false;
              });
              ...
              this.unsubscribeHistory = onSnapshot(qHistory, (snapshot) => {
                ...
                console.error('[CounterStore onSnapshot history] Error:', error);
              });
            } else {
  ...
  async updateCounterValue(counterId: string, newValue: number, description: string, isUndoRedo = false, method?: string) {
    ...
    const roundedNewValue = parseFloat(newValue.toFixed(counter.decimals));
    const delta = parseFloat((roundedNewValue - prevValue).toFixed(counter.decimals));
