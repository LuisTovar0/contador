<script lang="ts">
	...
	import { tooltip } from '../../lib/components/tooltip.svelte.ts';
	...
	let sortBy = $state<'updated' | 'name' | 'val-desc' | 'val-asc'>('updated');
...
<div class="w-full flex flex-col gap-6">
	...
	<!-- Toolbar Row (Search, Sorting, and View Switcher) -->
	{#if !counterStore.loading && counterStore.counters.length > 0}
		<div class="flex flex-col sm:flex-row gap-3 items-center justify-between w-full">
			...
			<div class="flex items-center gap-2.5 w-full sm:w-auto justify-end">
				...
				<div class="relative flex items-center bg-white/70 dark:bg-zinc-900/60 backdrop-blur-md border border-zinc-200/60 dark:border-purple-500/20 rounded-xl px-2.5 py-1.5 shadow-sm">
					<ArrowUpDown size={12} class="text-zinc-400 mr-2 animate-none" />
					<select
						bind:value={sortBy}
						class="bg-transparent text-xs font-semibold text-zinc-700 dark:text-zinc-300 focus:outline-none cursor-pointer pr-1"
					>
...
<Modal show={showLogoutWarningModal} onclose={() => (showLogoutWarningModal = false)}>
	...
	<div class="flex-1 overflow-y-auto space-y-3.5 text-xs md:text-sm leading-relaxed text-zinc-650 dark:text-zinc-300 pr-1 pb-2">
		<p>
			You are currently using the application in <strong>Anonymous Mode</strong>. Your data is stored locally on this device.
		</p>
		<p>
			Logging out will <strong>permanently delete all of your counters and history logs</strong>. This action cannot be undone.
		</p>
	</div>
